...
%nterm <std::shared_ptr<FermiStatementNode>> start
...
%%
start: statements {sourceFile.setSyntaxTree(std::make_unique<FermiStatementNode>($1));}
...
variable-declaration: 
    "let" IDENTIFIER "=" expression ";" {$$ = std::make_shared<VariableDeclarationStatementNode>(Type::deduced, $2, $4);}
    | "let" IDENTIFIER ":" type "=" expression ";" {$$ = std::make_shared<VariableDeclarationStatementNode>($4, $2, $6);}
    | "let" IDENTIFIER ":" type ";" {$$ = std::make_shared<VariableDeclarationStatementNode>($4, $2);}
    ;
type:
    "int8_t" {$$ = Type::int8_t;}
    ...
    ;
print_statement: "print" "(" expression-list ")" ";" {$$ = std::make_shared<PrintStatementNode>($3);};
...
expression:
    ...
    ;
creation_expression:
    expression "+" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Addition, $3);}
    | expression "-" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Subtraction, $3);}
    | expression "*" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Multiplication, $3);}
    | expression "/" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Division, $3);}
    | expression "//" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::IntegerDivision, $3);}
    | expression "^" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Exponentiation, $3);}
    | expression "%" expression {$$ = std::make_shared<BinaryExpressionNode>($1, BinaryExpressionOperators::Modulo, $3);}
    ;
identity_expression:
    literal {$$ = $1;}
    | IDENTIFIER {$$ = std::make_shared<LiteralExpressionNode>($1, LiteralType::Identifier);}
    | "(" expression ")" {$$ = $2;}
    ;
literal: 
    INTEGER_LITERAL {$$ = std::make_shared<LiteralExpressionNode>($1, LiteralType::Integer);}
    | FLOAT_LITERAL {$$ = std::make_shared<LiteralExpressionNode>($1, LiteralType::Float);}
    ;
%%
namespace Fermi::SyntaxAnalysis
{
